...
2. Show the report. Safe ops are index rebuild, empty-file delete, and identical-body merges.
3. If the user did not pass `apply` as an argument, stop after the dry-run unless they confirm.
4. Call `memory_dream` with `dryRun: false` to apply **safe** ops only.
...
6. `memory_index` at the end. Confirm the index lines you changed.
